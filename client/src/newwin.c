...
extern void redrawPStats(void), redrawStats(void);
...
void redrawTeam(W_Window win, int teamNo, int *lastnum);
...
newwin(char *hostmon, char *progname)
{
  ...
  playerw = W_MakeTextWindow("player", 0, YOFF + WINSIDE + 2 * BORDER + 2 * MESSAGESIZE,
                             PlistMaxWidth(), MAXPLAYER + 3, NULL, 2);
  ...
  qwin = W_MakeWindow("quit", 4 * BOXSIDE, WINSIDE - BOXSIDE, BOXSIDE, BOXSIDE, w, 1,
                      foreColor);

#ifdef ARMY_SLIDER
  statwin = W_MakeWindow("stats", 422, 13, 160, 110, NULL, BORDER, foreColor);
#else
  statwin = W_MakeWindow("stats", 422, 13, 160, 95, NULL, BORDER, foreColor);
#endif /* ARMY_SLIDER */
...
{
  int i,j,k;
  HINSTANCE hShipLibrary;
  HINSTANCE hWeapLibrary;
  HINSTANCE hMiscLibrary;
  HINSTANCE hPlanLibrary;
  
  if (colorClient) {
    if ((hShipLibrary = LoadLibrary("SHIPLIB.DLL")) == NULL)
      {
  	fprintf(stderr, "Could not open shiplib.dll\n");
  	exit(1);
      }
  }
  else {
    if ((hShipLibrary = LoadLibrary("SHIPLIBM.DLL")) == NULL)
      {
  	fprintf(stderr, "Could not open shiplibm.dll\n");
  	exit(1);
      }
  }
  ...
  if (colorClient) {
    if ((hMiscLibrary = LoadLibrary("MISCLIB.DLL")) == NULL)
      {
  	fprintf(stderr, "Could not open misclib.dll\n");
  	exit(1);
      }
  }
  else {
    if ((hMiscLibrary = LoadLibrary("MISCLIBM.DLL")) == NULL)
      {
  	fprintf(stderr, "Could not open misclibm.dll\n");
  	exit(1);
      }
  }

  if ((hPlanLibrary = LoadLibrary("PLANLIB.DLL")) == NULL)
    {
	fprintf(stderr, "Could not open planlib.dll\n");
	exit(1);
    }

  if (colorClient) {
    ship_bitmaps[0] = 
        W_StoreBitmap2(hShipLibrary, BMP_SHIP_WIDTH*8, BMP_SHIP_HEIGHT*32,
                       BMP_FED_SHIP, w, LR_DEFAULTCOLOR);
    ship_bitmaps[1] =
      W_StoreBitmap2(hShipLibrary, BMP_SHIP_WIDTH*8, BMP_SHIP_HEIGHT*32,
                     BMP_IND_SHIP, w, LR_DEFAULTCOLOR);
    ship_bitmaps[2] =
      W_StoreBitmap2(hShipLibrary, BMP_SHIP_WIDTH*8, BMP_SHIP_HEIGHT*32,
                     BMP_KLI_SHIP, w, LR_DEFAULTCOLOR);
    ship_bitmaps[3] =
      W_StoreBitmap2(hShipLibrary, BMP_SHIP_WIDTH*8, BMP_SHIP_HEIGHT*32,
                     BMP_ORI_SHIP, w, LR_DEFAULTCOLOR);
    ship_bitmaps[4] =
      W_StoreBitmap2(hShipLibrary, BMP_SHIP_WIDTH*8, BMP_SHIP_HEIGHT*32,
                     BMP_ROM_SHIP, w, LR_DEFAULTCOLOR);
      ...
      for (j = 0; j < NUM_TYPES; j++)
        ...
        for (k = 0; k < SHIP_VIEWS; k++)
          {
            fed_bitmaps[j][k] = 
             W_PointBitmap2(ship_bitmaps[0], j, k, BMP_SHIP_WIDTH, BMP_SHIP_HEIGHT);
           ind_bitmaps[j][k] = 
             W_PointBitmap2(ship_bitmaps[1], j, k, BMP_SHIP_WIDTH, BMP_SHIP_HEIGHT);
           kli_bitmaps[j][k] = 
             W_PointBitmap2(ship_bitmaps[2], j, k, BMP_SHIP_WIDTH, BMP_SHIP_HEIGHT);
           ori_bitmaps[j][k] = 
             W_PointBitmap2(ship_bitmaps[3], j, k, BMP_SHIP_WIDTH, BMP_SHIP_HEIGHT);
           rom_bitmaps[j][k] = 
             W_PointBitmap2(ship_bitmaps[4], j, k, BMP_SHIP_WIDTH, BMP_SHIP_HEIGHT);
          }
        }
    }
  else {
    ship_bitmaps[0] = 
        W_StoreBitmap2(hShipLibrary, BMP_SHIP_WIDTH*8, BMP_SHIP_HEIGHT*32,
                       BMP_FED_SHIP, w, LR_MONOCHROME);
    ship_bitmaps[1] =
      W_StoreBitmap2(hShipLibrary, BMP_SHIP_WIDTH*8, BMP_SHIP_HEIGHT*32,
                     BMP_IND_SHIP, w, LR_MONOCHROME);
    ship_bitmaps[2] =
      W_StoreBitmap2(hShipLibrary, BMP_SHIP_WIDTH*8, BMP_SHIP_HEIGHT*32,
                     BMP_KLI_SHIP, w, LR_MONOCHROME);
    ship_bitmaps[3] =
      W_StoreBitmap2(hShipLibrary, BMP_SHIP_WIDTH*8, BMP_SHIP_HEIGHT*32,
                     BMP_ORI_SHIP, w, LR_MONOCHROME);
    ship_bitmaps[4] =
      W_StoreBitmap2(hShipLibrary, BMP_SHIP_WIDTH*8, BMP_SHIP_HEIGHT*32,
                     BMP_ROM_SHIP, w, LR_MONOCHROME);
  ...
  clockpic = W_StoreBitmap2(hMiscLibrary, BMP_CLOCK_WIDTH, BMP_CLOCK_HEIGHT, BMP_CLOCK, qwin, LR_MONOCHROME);
...
/* Experimental weapons */
  ...
  for (i = 0; i < BMP_TORPDET_FRAMES; i++)
    {
     cloud[i] = W_StoreBitmap2(hWeapLibrary, BMP_TORPDET_WIDTH, BMP_TORPDET_HEIGHT,
                       BMP_CLOUD + i, w, LR_MONOCHROME);

     plasmacloud[i] = W_StoreBitmap2(hWeapLibrary, BMP_PLASMATORPDET_WIDTH, BMP_PLASMATORPDET_HEIGHT,
                        BMP_PLCLOUD + i, w, LR_MONOCHROME);

     }

     etorp = W_StoreBitmap2(hWeapLibrary, BMP_TORP_WIDTH, BMP_TORP_HEIGHT, BMP_ETORP, w, LR_MONOCHROME);
     mtorp = W_StoreBitmap2(hWeapLibrary, BMP_TORP_WIDTH, BMP_TORP_HEIGHT, BMP_MTORP, w, LR_MONOCHROME);
     eplasmatorp = W_StoreBitmap2(hWeapLibrary, BMP_EPLASMA_WIDTH, BMP_EPLASMA_HEIGHT, BMP_EPLASMA, w, LR_MONOCHROME);
     mplasmatorp = W_StoreBitmap2(hWeapLibrary, BMP_MPLASMA_WIDTH, BMP_MPLASMA_HEIGHT, BMP_MPLASMA, w, LR_MONOCHROME);

#endif /* COLORIZEWEAPON */
  

  bplanets[0] = W_StoreBitmap2(hPlanLibrary, BMP_PLANET_WIDTH, BMP_PLANET_HEIGHT, BMP_PLANET000, w, LR_MONOCHROME);
  bplanets[1] = W_StoreBitmap2(hPlanLibrary, BMP_PLANET_WIDTH, BMP_PLANET_HEIGHT, BMP_PLANET001, w, LR_MONOCHROME);
  bplanets[2] = W_StoreBitmap2(hPlanLibrary, BMP_PLANET_WIDTH, BMP_PLANET_HEIGHT, BMP_PLANET010, w, LR_MONOCHROME);
  bplanets[3] = W_StoreBitmap2(hPlanLibrary, BMP_PLANET_WIDTH, BMP_PLANET_HEIGHT, BMP_PLANET011, w, LR_MONOCHROME);
  bplanets[4] = W_StoreBitmap2(hPlanLibrary, BMP_PLANET_WIDTH, BMP_PLANET_HEIGHT, BMP_PLANET100, w, LR_MONOCHROME);
  bplanets[5] = W_StoreBitmap2(hPlanLibrary, BMP_PLANET_WIDTH, BMP_PLANET_HEIGHT, BMP_PLANET101, w, LR_MONOCHROME);
  bplanets[6] = W_StoreBitmap2(hPlanLibrary, BMP_PLANET_WIDTH, BMP_PLANET_HEIGHT, BMP_PLANET110, w, LR_MONOCHROME);
  bplanets[7] = W_StoreBitmap2(hPlanLibrary, BMP_PLANET_WIDTH, BMP_PLANET_HEIGHT, BMP_PLANET111, w, LR_MONOCHROME);
  bplanets[8] = W_StoreBitmap2(hPlanLibrary, BMP_PLANET_WIDTH, BMP_PLANET_HEIGHT, BMP_PLANETUNKN, w, LR_MONOCHROME);

  bmplanets[0] = W_StoreBitmap2(hPlanLibrary, BMP_MPLANET_WIDTH, BMP_MPLANET_HEIGHT, BMP_MPLANET000, mapw, LR_MONOCHROME);
  bmplanets[1] = W_StoreBitmap2(hPlanLibrary, BMP_MPLANET_WIDTH, BMP_MPLANET_HEIGHT, BMP_MPLANET001, mapw, LR_MONOCHROME);
  bmplanets[2] = W_StoreBitmap2(hPlanLibrary, BMP_MPLANET_WIDTH, BMP_MPLANET_HEIGHT, BMP_MPLANET010, mapw, LR_MONOCHROME);
  bmplanets[3] = W_StoreBitmap2(hPlanLibrary, BMP_MPLANET_WIDTH, BMP_MPLANET_HEIGHT, BMP_MPLANET011, mapw, LR_MONOCHROME);
  bmplanets[4] = W_StoreBitmap2(hPlanLibrary, BMP_MPLANET_WIDTH, BMP_MPLANET_HEIGHT, BMP_MPLANET100, mapw, LR_MONOCHROME);
  bmplanets[5] = W_StoreBitmap2(hPlanLibrary, BMP_MPLANET_WIDTH, BMP_MPLANET_HEIGHT, BMP_MPLANET101, mapw, LR_MONOCHROME);
  bmplanets[6] = W_StoreBitmap2(hPlanLibrary, BMP_MPLANET_WIDTH, BMP_MPLANET_HEIGHT, BMP_MPLANET110, mapw, LR_MONOCHROME);
  bmplanets[7] = W_StoreBitmap2(hPlanLibrary, BMP_MPLANET_WIDTH, BMP_MPLANET_HEIGHT, BMP_MPLANET111, mapw, LR_MONOCHROME);
  bmplanets[8] = W_StoreBitmap2(hPlanLibrary, BMP_MPLANET_WIDTH, BMP_MPLANET_HEIGHT, BMP_MPLANETUNKN, mapw, LR_MONOCHROME);


  if (colorClient) {
    for (i = 0; i < BMP_SHIPEXPL_FRAMES; i++)
      {
        expview[i] = W_StoreBitmap2(hMiscLibrary, BMP_SHIPEXPL_WIDTH, BMP_SHIPEXPL_HEIGHT, BMP_SHIP_EXPLOSION+i, w, LR_DEFAULTCOLOR);
      }
    for (i = 0; i < BMP_SBEXPL_FRAMES; i++)
      {
        sbexpview[i] = W_StoreBitmap2(hMiscLibrary, BMP_SBEXPL_WIDTH, BMP_SBEXPL_HEIGHT, BMP_SB_EXPLOSION+i, w, LR_DEFAULTCOLOR);
      }
  }
  else {
    for (i = 0; i < BMP_SHIPEXPL_FRAMES; i++)
      {
        expview[i] = W_StoreBitmap2(hMiscLibrary, BMP_SHIPEXPL_WIDTH, BMP_SHIPEXPL_HEIGHT, BMP_SHIP_EXPLOSION+i, w, LR_MONOCHROME);
      }
    for (i = 0; i < BMP_SBEXPL_FRAMES; i++)
      {
        sbexpview[i] = W_StoreBitmap2(hMiscLibrary, BMP_SBEXPL_WIDTH, BMP_SBEXPL_HEIGHT, BMP_SB_EXPLOSION+i, w, LR_MONOCHROME);
      }
  }

#ifndef VSHIELD_BITMAPS
  shield = W_StoreBitmap2(hMiscLibrary, BMP_SHIELD_WIDTH, BMP_SHIELD_HEIGHT, BMP_SHIELD, w, LR_MONOCHROME);
#else
  for (i = 0; i < SHIELD_FRAMES; i++)
    shield[i] = W_StoreBitmap2(hMiscLibrary, BMP_SHIELD_WIDTH, BMP_SHIELD_HEIGHT, BMP_VSHIELD+i, w, LR_MONOCHROME);
#endif

  cloakicon = W_StoreBitmap2(hMiscLibrary, BMP_CLOAK_WIDTH, BMP_CLOAK_HEIGHT, BMP_CLOAK, w, LR_MONOCHROME);
  icon = W_StoreBitmap2(hMiscLibrary, BMP_ICON_WIDTH, BMP_ICON_HEIGHT, BMP_ICON, iconWin, LR_MONOCHROME);
  stipple = W_StoreBitmap2(hMiscLibrary, BMP_STIPPLE_WIDTH, BMP_STIPPLE_HEIGHT, BMP_STIPPLE, w, LR_MONOCHROME);
...
void entrywindow(int *team, int *s_type)
{
  ...
  *team = -1;
  ...
  quittime = (time_t) intDefault("autoquit", quittime); /* allow extra */
  /* quit time -RW */
...
showTimeLeft(time_t time, time_t max)
{
  ...
  ex = cx - BMP_CLOCK_WIDTH * sin(2 * XPI * time / max) / 2;
  ey = cy - BMP_CLOCK_HEIGHT * cos(2 * XPI * time / max) / 2;
