...
W_Icon W_StoreBitmap2(HINSTANCE hDLLInstance, int width,int height,int bits,W_Window window, UINT BmpType)
   {
   struct Icon *bitmap=0;
   HBITMAP temp=0;
...
//   if (! (temp = LoadImage(hDLLInstance, MAKEINTRESOURCE(bits), IMAGE_BITMAP, 0, 0, LR_MONOCHROME)))
   if (! (temp = LoadImage(hDLLInstance, MAKEINTRESOURCE(bits), IMAGE_BITMAP, 0, 0, BmpType)))
...
memfail:
   printf("Memory allocation or CreateBitmap() failure in StoreBitmap2()\n");
   printf("Id: %d %d %d\n", height, width, bits);
   if (bitmap) free(bitmap);
   if (temp) DeleteObject(temp);
   return NULL;
...
// This Bitmap drawing function is for the 256 color bitmaps
// Try to do Transparency masking
void W_WriteBitmap2(int x, int y, W_Icon icon, W_Color color)
   {
   register struct Icon *bitmap = (struct Icon *)icon;
   register int border,width,height;
   register int srcx,srcy;
...
// Set up transparency mask
   hbmTransMask = CreateBitmap(width,height,1,1,NULL);

   SelectObject(GlobalMemDC, bitmap->bm );
   SelectObject(hdcMem2, hbmTransMask);
   
   //Set background color to transparency color( it's black)
   SetBkColor(GlobalMemDC, RGB(0,0,0));
   // Create the transparency
   BitBlt(hdcMem2, 0, 0, width, height, GlobalMemDC, 0, 0, SRCCOPY);

   // Now draw the bitmap   
//   BitBlt(hdc, x, y, width, height, GlobalMemDC, srcx, srcy, SRCINVERT);
   BitBlt(hdc, x, y, width, height, hdcMem2, srcx, srcy, SRCCOPY);
   BitBlt(hdc, x, y, width, height, GlobalMemDC, srcx, srcy, SRCPAINT);
