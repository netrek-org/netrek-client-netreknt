  ...
  for (l = planets + MAXPLANETS - 1; l >= planets; --l)
    {
      ...
      /* Draw the new planet */
          ...
          W_OverlayBitmap(dx - (BMP_MPLANET_WIDTH / 2), dy - (BMP_MPLANET_HEIGHT / 2),
      ...
      if (showIND && (l->pl_info & me->p_team) && (l->pl_owner == NOBODY))
        {
          W_MakeLine(mapw, dx + (BMP_MPLANET_WIDTH / 2 - 1),
                     ...
                     dx - (BMP_MPLANET_WIDTH / 2), dy - (BMP_MPLANET_HEIGHT / 2),
                     W_White);
          W_MakeLine(mapw, dx - (BMP_MPLANET_WIDTH / 2),
                     dy + (BMP_MPLANET_HEIGHT / 2 - 1),
                     dx + (BMP_MPLANET_WIDTH / 2 - 1), dy - (BMP_MPLANET_HEIGHT / 2),
                     W_White);
        }
